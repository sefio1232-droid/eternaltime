...
describe("catalog Phase 3.2 showroom recovery", () => {
  ...
  // 595 is the verified public count after adding the 73-row Seiko Women staged import and
  // excluding 25 Seiko LUKIA records from public storefront surfaces while keeping raw/import data.
  it("10. the All tab preserves the real catalog's full 595 public records", () => {
    const all = listCatalogWatches(realDataset(), parseCatalogReadQuery({ searchParams: { view: "all" } }));
    expect(all.totalRecords).toBe(595);
  });
